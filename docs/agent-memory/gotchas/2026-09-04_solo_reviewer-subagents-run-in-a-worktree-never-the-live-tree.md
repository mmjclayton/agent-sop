...
**Second occurrence, 2026-09-05 (P103).** Six reviewers were given a detached worktree *path* in their prompts and told not to write anywhere. The pr-test-analyzer still built a "throwaway copy" and ran its mutations with its shell in the live repo: eight-line `Backlog.md`, one-line `CLAUDE.md`, gutted core SOP, stray `src.js`. Caught by an anchor assertion in the session's own Backlog edit; restored with `git checkout --`, nothing committed. The lesson sharpened: a path in the prompt is advice; the Agent tool's `isolation: "worktree"` is the control, because it moves the agent's own cwd. Use it for every reviewer that has Bash.
