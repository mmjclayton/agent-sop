...
**First consumer run + P101.** A peer session in opportunity-scan (launched from `~`, moved in with `cd`) reported cross-session: context block on the first prompt after the move, Stop hook demanded the ship-sop gate once, gate run started. Its observation that the block showed "Drift: none" while a gate was outstanding became P101: the block now carries a `Ship gate:` line from the same rule. Batch 0.39.
