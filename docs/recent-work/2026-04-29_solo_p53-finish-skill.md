...
New slash command at `.claude/commands/finish.md` (mirrored to `~/.claude/commands/finish.md`). Three hard-blocking phases that run when Claude believes the work is done:
...
Motivation: passing types and unit tests is not the same as exercising the change. `/finish` makes Claude prove the work runs against the real surface before the SOP trail and PR get created.

Originally shipped as `/go` and renamed to `/finish` later the same day — `/go` collided with the Go language and read as imperative without context; `/finish` matches the framing ("the 'I'm done' wrapper").
...
- `.claude/commands/finish.md` (new — canonical, ships with SOP installs; renamed from `go.md` later)
- `~/.claude/commands/finish.md` (user-scope mirror; renamed from `go.md` later)
...
- No follow-ups filed. `/finish` will dogfood itself the next time a code change ships through this repo (or any other ECC project).
