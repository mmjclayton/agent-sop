...
**Hand-edits to the rollup.** The rollup in `docs/RECENT-WORK.md` is regenerated by `/update-sop` Step 8b from `docs/recent-work/`. Hand-edits get overwritten silently. **Edit the source file in `docs/recent-work/` and re-run `/update-sop`.**
