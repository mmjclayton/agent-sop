...
# Review template — pristine replica, referenced by /update-sop Step 2 (review)
copy_if_missing "$TEMPLATE_DIR/review-template.md" "$TARGET/docs/templates/review-template.md"
# Review artefacts (Step 2) are written here — pre-create so first invocation does not fail on missing dir
...
\`scripts/refresh-rollup.sh\` (called from \`/update-sop\` Step 6). Do not hand-edit
