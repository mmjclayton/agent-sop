...
review: docs/reviews/2026-09-05_solo_P105.md (shipped as part of P105)
...
Reason: the historical mandatory benchmark rule is retired under P110. Current evaluation follows docs/benchmark/evaluation-protocol.md; fixture obligations remain. Original diagnosis retained below.
...
`[WON'T] [Bug] — Reason: the Step 2a collision check was removed in P105 (never fired in five months, cost a fetch per session)`
...
`[WON'T] [Iteration] — Reason: Rule 5 (instruction budget) was removed in P105; the trim itself is the outcome`
...
- `scripts/sync-sop-files.sh`: reads the manifest table from the upstream copy of the command, classifies every file (IN-SYNC, MISSING, OLDER, MODIFIED, RECONCILE, EXCLUDED); OLDER means the consumer copy equals the baseline or any past upstream version of the file in git history. `--apply` writes MISSING and OLDER, copies the executable bit, refreshes `baseline_shas` and `last_update_check` through a temp file, and leaves a `.bak` of the config. Dry run by default.
- `/update-agent-sop` Steps 1-4 call it; the GitHub-raw fallback keeps the prose three-way for a machine without the checkout; RECONCILE stays a question to the operator.
...
- `docs/sop/claude-agent-sop.md` 791 → ~110 lines: Rules 1 and 2, the file map with the resolver rule once, entry formats, project type, size rules, session start and the seven-step close, tags and transitions, CLAUDE.md requirements, companions. Rules 3-6, §3 file specs, §4, §7, §10 issue sync, §13, §15.2-15.5, §17 and every rationale paragraph dropped; the incidents they cited are named inline by P-number.
- `/update-sop` 669 → ~110 lines, seven steps, each a script call or a checkable product; Step 2 is one review run that serves the ship gate too; the P-number collision pre-check (never fired, one `git fetch` per session), the Definition of Done, feature-map, Batch Log, MEMORY.md and branch-prune steps are gone. `/restart-sop` 323 → ~40; `/finish` 200 → ~30.
- `scripts/validate-state-transitions.sh`: the review citation (`review: docs/reviews/...`, path checked) or skip token is read from the Backlog entry, not a Batch Log line; no Batch Log reference is required for `[SHIPPED]`. Fixtures moved with it (20 cases; the old no-Batch-Log case is now a legal Iteration ship).
- Templates: Definition of Done, Deprioritised, Code Quality Rules and the checklist copies removed; priority sentinels in; ~120 and ~170 lines. This repo's CLAUDE.md follows suit (sentinels live, intent-based dispatch).
- `docs/sop/compliance-checklist.md`: checks for the dropped artefacts removed (IDs never reused), B13 and C25 added; 79 code / 73 non-code.
- `scripts/archive-backlog.sh`: closed items older than 90 days move verbatim to `docs/backlog-archive.md` with pointers left behind; applied here (53 items, 2198 → 1170 lines).
...
- `sop-session-context.sh`: in-flight, in-progress, drift, dirty-tracker, ship-gate, worktree and sync lines print only when they are not the default; the closing line drops the enforcement-status sentence. The audit measured roughly 700 of 1,322 bytes as "(none)" lines on a clean repo.
- `resolve-resume-path.sh --read`: a legacy `project_resume.md` whose first non-blank line carries the `**SUPERSEDED` marker (BOM and CR stripped) is never served; prose that merely contains the word still resolves (review: the first cut was an unanchored `head -1` match) (the audit saw a 3,503-byte superseded snapshot injected when the agent-id had no per-agent file).
...
- `sop_is_push_command` ignores here-document bodies: a command that writes a file whose text mentions a push verb is not a push (the gate refused a scratchpad write during this session).
...
- P1 — archived Core SOP document: see docs/backlog-archive.md
- P2 — archived CLAUDE.md base template: see docs/backlog-archive.md
- P11 — archived CLAUDE.md code project template: see docs/backlog-archive.md
- P3 — archived Agent memory template: see docs/backlog-archive.md
- P4 — archived Backlog template: see docs/backlog-archive.md
- P5 — archived Build plan template: see docs/backlog-archive.md
- P6 — archived New project walkthrough: see docs/backlog-archive.md
- P7 — archived Existing project migration guide: see docs/backlog-archive.md
- P12 — archived SOP v2: owner feedback iteration: see docs/backlog-archive.md
- P13 — archived SOP Compliance Checker Agent: see docs/backlog-archive.md
- P14 — archived Security guidance document: see docs/backlog-archive.md
- P15 — archived Hooks guidance document: see docs/backlog-archive.md
- P16 — archived Code quality rules in code template: see docs/backlog-archive.md
- P17 — archived Reference agent definitions: see docs/backlog-archive.md
- P18 — archived Expand code template sections: see docs/backlog-archive.md
- P19 — archived Continuous learning pattern: see docs/backlog-archive.md
- P20 — archived Compliance checklist update (security, hooks, quality, agents): see docs/backlog-archive.md
- P21 — archived Setup script for new projects: see docs/backlog-archive.md
- P22 — archived Session slash commands (/restart-sop, /update-sop): see docs/backlog-archive.md
- P23 — archived SOP Benchmark Framework: see docs/backlog-archive.md
- P29 — archived Pre-launch README polish + LICENSE + minimum version note: see docs/backlog-archive.md
- P30 — archived Research digest review (2026-04-13) — verdict only, no implementation bundle: see docs/backlog-archive.md
- P43 — archived Parallel multi-agent session support: see docs/backlog-archive.md
- P42 — archived Secondary-tracker reconciliation + [DEFERRED] tag: see docs/backlog-archive.md
- P41 — archived README rewrite, License section, Acknowledgements removed, GitHub About refresh: see docs/backlog-archive.md
- P40 — archived Section 14 + Section 15.4 trim, CLAUDE.md Recent Work + agent-memory.md Decisions compaction: see docs/backlog-archive.md
- P39 — archived Measurement gap closed: hygiene rubric + continuity benchmark + longitudinal exhibit: see docs/backlog-archive.md
- P38 — archived R5 post-trim benchmark + README claim audit: see docs/backlog-archive.md
- P37 — archived claude-mem review findings applied: see docs/backlog-archive.md
- P36 — archived SOP sync mechanism (/update-agent-sop): see docs/backlog-archive.md
- P35 — archived Section 4 Versioning Rules removed (pure duplicate): see docs/backlog-archive.md
- P34 — archived Rule 1 extended; Rule 6 added; failure-mode annotations: see docs/backlog-archive.md
- P32 — archived SOP instruction-budget trim: see docs/backlog-archive.md
- P44 — archived Required reviewer turn before ship (with substance assertion): see docs/backlog-archive.md
- P45 — archived State-transition validator: see docs/backlog-archive.md
- P46 — archived Mid-session drift detection (actionable, not informational): see docs/backlog-archive.md
- P48 — archived Reviewer voice rules + Backlog item-sizing pedagogy: see docs/backlog-archive.md
- P49 — archived Instrument `/update-sop` step timing before any trim refactor: see docs/backlog-archive.md
- P51 — archived Safe optimisations to `/restart-sop` read phase (parallel reads + targeted Backlog load): see docs/backlog-archive.md
- P52 — archived Learnings capture pattern (doc-only, `/update-sop` integration): see docs/backlog-archive.md
- P54 — archived Multi-agent hardening + perf gates + worktree advisory: see docs/backlog-archive.md
- P47 — archived Drift check: resume-file fallback fails on multi-worktree projects with legacy unsuffixed resume: see docs/backlog-archive.md
- P24 — archived Multi-agent optimisation guide: see docs/backlog-archive.md
- P25 — archived Incorporate benchmark findings into SOP: see docs/backlog-archive.md
- P26 — archived Benchmark-driven SOP optimisations: see docs/backlog-archive.md
- P27 — archived Managed Agents integration and outcome rubrics: see docs/backlog-archive.md
- P28 — archived Research digest implementation (v2.1.97, context management, evolution loop): see docs/backlog-archive.md
- P53 — archived `/finish` skill: end-to-end verify, simplify, ship: see docs/backlog-archive.md
- P55 — archived Sycophantic reviewer detection: tighten substance assertion: see docs/backlog-archive.md
- P57 — archived Config `exclude` field for `/update-agent-sop`: see docs/backlog-archive.md
...
Merged via PR #30 (2d4cf7d) with ship-sop P33 (PR #16, 1ace2d8); CI green on main in both. User-scope Claude and Codex hooks refreshed the same day.
...
**Acceptance criteria:**
- Hook fixtures: a version-2 receipt without counts does not cover; one whose block rounds exceed its runs does not cover; one with counts covers; existing version-1 cases unchanged
- shellcheck clean; all nine suites pass
